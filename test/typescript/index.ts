...
const scopes: Scopes = {
	...
	byAuthor: (query: Record<string, any>, ctx: Context, params: any) => {
		if (params.authorId) {
...
const postServiceSchema: ServiceSchema<
	DatabaseServiceSettings,
	DatabaseMethods,
	DatabaseLocalVariables
> = {
...
function demonstrateServiceUsage() {
	...
	return {
		...
		async findByAuthor(authorId: string) {
			const posts = await broker.call<Post[], { authorId: string; limit: number }>(
				"posts.findByAuthor",
				{
					authorId,
					limit: 5
...
function demonstrateDirectServiceMethods(service: MoleculerService & DatabaseMethods) {
	return {
		...
		async testEntityMethods(ctx: Context) {
			...
			const createdPosts = await service.createEntities<Post>(
				ctx,
				[
...
const multiTenantServiceSchema: ServiceSchema<
	DatabaseServiceSettings,
	DatabaseMethods,
	DatabaseLocalVariables
> = {
	...
	settings: {
		...
		scopes: {
			tenant: (query: Record<string, any>, ctx: Context, params: any) => {
...
const advancedServiceSchema: ServiceSchema<
	DatabaseServiceSettings,
	DatabaseMethods,
	DatabaseLocalVariables
> = {
	...
	settings: {
		...
		scopes: {
			...
			byUser: (
				...
				ctx: Context<any, { user: { id: string } }>,
				params: any
			) => {
				if (ctx.meta?.user?.id) {
	...
	events: {
		...
		"posts.updated"(ctx: Context<{ entity: Post; oldEntity: Post }>) {
			...
			if (
				ctx.params.oldEntity.status !== ctx.params.entity.status &&
				ctx.params.entity.status
			) {
